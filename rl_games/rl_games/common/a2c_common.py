...
def print_statistics(print_stats, curr_frames, step_time, step_inference_time, total_time, epoch_num, max_epochs, frame, max_frames):
    if print_stats:
        ...
        # print(f'curr_frames:{curr_frames} step_time:{step_time} step_inference_time:{step_inference_time} total_time:{total_time}')
...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        # sim-system need run few step before reset by rlgames
        print("sim-system need run few step before reset by rlgames")
        for i in range(5):
            self.vec_env.env._world.step(render=False)
            self.vec_env.env._task.update_state()
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            # 如果使用中央价值网络，将状态更新到经验缓冲区
            if self.has_central_value:
                self.experience_buffer.update_data('states', n, self.obs['states'])

            # 记录环境交互开始时间
            step_time_start = time.time()
            # 执行环境步骤，获取下一个观测、奖励、完成状态和信息
            ...
            # 将未完成环境的当前奖励保留，已完成环境的奖励清零
            self.current_rewards = self.current_rewards * not_dones.unsqueeze(1)
            # 将未完成环境的当前调整奖励保留，已完成环境的奖励清零
            self.current_shaped_rewards = self.current_shaped_rewards * not_dones.unsqueeze(1)
            # 将未完成环境的当前长度保留，已完成环境的长度清零
...
class ContinuousA2CBase(A2CBase):
    ...
    # TODO: Implement train_epoch method
    def train_epoch(self):
        # 调用父类的train_epoch方法
        super().train_epoch()
    ...
    def prepare_dataset(self, batch_dict):
        ...
        # 提取RNN状态，如果不存在则设为None
        rnn_states = batch_dict.get('rnn_states', None)
        # 提取RNN掩码，如果不存在则设为None
        rnn_masks = batch_dict.get('rnn_masks', None)
    ...
    def train(self):
        # 初始化张量
        self.init_tensors()
        # 设置上次平均奖励的初始值：初始化为一个很小的值，用于比较新旧奖励
