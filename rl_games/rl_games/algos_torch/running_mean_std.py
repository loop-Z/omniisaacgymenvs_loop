...
class DummyRunningMeanStd(nn.Module):
    def __init__(self, insize, epsilon=1e-05, per_channel=False, norm_only=False):
        super(DummyRunningMeanStd, self).__init__()
        self.insize = insize
...
class RunningMeanStdObs(nn.Module):
    def __init__(self, insize, epsilon=1e-05, per_channel=False, norm_only=False, keys=[]):
        ...
        if keys:
            ...
            for k,v in insize.items():
                if k in keys:
                    self.running_mean_std[k] = RunningMeanStd(v, epsilon, per_channel, norm_only)
                else:
                    self.running_mean_std[k] = DummyRunningMeanStd(v)
        else:
            self.running_mean_std = nn.ModuleDict({
