...
class NetworkBuilder:
    ...
    class BaseNetwork(nn.Module):
        ...
        def _build_mlp(
            self,
            input_size,      # MLP网络的输入维度
            units,           # 包含各隐藏层神经元数量的列表，例如[64, 128, 64]
            activation,      # 激活函数名称，如'relu', 'tanh'等
            dense_func,      # 全连接层函数，通常是torch.nn.Linear
            norm_only_first_layer=False,   # 是否只对第一层应用归一化
            norm_func_name=None,           # 归一化函数名称，如'layer_norm', 'batch_norm'
            d2rl=False,                    # 是否使用D2RL（Distributional Deep Reinforcement Learning）网络
        ):
            if d2rl:
                act_layers = [
