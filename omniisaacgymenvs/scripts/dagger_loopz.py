...
from omniisaacgymenvs.algo.ppo.dagger import DaggerExpert, DaggerAgent, DaggerTrainer
...
for update in range(1201):
    ...
    if update %  dagger_cfg['environment']['eval_every_n'] == 0:
        ...
        actor.save_deterministic_graph(saver.data_dir+"/prop_encoder_"+str(update)+'.pt', 
                                       ...
                                       torch.rand(1, ob_dim - priv_dim + latent_dim).cpu())
    ...
    print('{:<40} {:>6}'.format("latent mse loss: ", '{:0.10f}'.format(mse_loss)))
