...
class USVVirtual(RLTask):
    def __init__(
        ...
        self._green_markers = [None] * self.task.n_dyn
        #self.set_targets(torch.arange(self._num_envs, device=self._device))
        self.enable_viewport = self._cfg.get("enable_viewport", True)
    ...
    def update_state(self) -> None:
        self.task.move_dynamic_obstacles(self.dt)
        
        # dyn_local_pos = self.task.dyn_obs_pos # (N, n_dyn, 3)
        # if self._green_markers[0]:
        #     for i in range(self.task.n_dyn):
        #         global_pos = dyn_local_pos[:, i, :] + self._env_pos
        #         orientations = torch.zeros((self._num_envs, 4), device=self._device)
        #         orientations[:, 0] = 1.0 
                
        #         self._green_markers[i].set_world_poses(
        #             global_pos,
        #             orientations,
...
########################################################可视化变化
    ...
    def set_up_scene(self, scene) -> None:
        ...
        scene, self._marker, self._blue_markers, self._green_markers = self.task.add_visual_marker_to_scene(scene)
        return
