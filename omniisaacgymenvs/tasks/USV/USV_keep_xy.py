...
class KeepXYTask(Core):
    """
    Implements the KeepXY task. The robot has to reach a target position."""

    def __init__(
        self,
        task_param: KeepXYParameters,
    ...
    def compute_reward(
        ...
        # position error# 1. 计算当前位置误差（欧几里得距离）
        ...
        positionposition=self.positionposition
        print("position",positionposition)
...
#        self.positionposition=position
    ...
    def generate_target(self, path, position):
        ...
        self.positionposition=position
        water_z = 0.24          # heron_zero_height 120 mm
        cylinder_height = 0.02   # # 圆盘厚度

        #  # 创建红色圆柱体（当圆盘用）
        prim_utils.create_prim(
            prim_path=path + "/pin",
            prim_type="Cylinder",
            translation=(float(position[0]), # x 坐标
                        float(position[1]),
                        1), # z 坐标：圆柱中心 = 水面 - 厚度 / 2
            scale=(2.0, 2.0, cylinder_height),# (x, y, z) 缩放：2 m 半径，2 cm 厚
            attributes={"primvars:displayColor": [(1.0, 0.0, 0.0)]}# 纯红色
        ) #
    ...
    # def generate_target(self, path, position):
    #     """
    #     区域锚：1 m 半径、2 cm 厚红色圆盘
    #     纯 create_prim，不碰 PreviewSurface，路径仍为 /pin
    #     """
    #     import omni.isaac.core.utils.prims as prim_utils

    #     prim_utils.create_prim(
    #         prim_path=path + "/pin",
    #         prim_type="Cylinder",                       # 圆柱当圆盘
    #         translation=(float(position[0]),
    #                     float(position[1]),
    #                     0.01),                         # 贴水面
    #         scale=(1.0, 1.0, 0.02),                     # 半径 1 m，厚度 0.02 m
    #         attributes={"primvars:displayColor": [(1.0, 0.0, 0.0)]}  # 红色
    #     )
